...
func (Kernel) Boot(ctx context.Context) error {
	...
	ki.Dest = paths.KBoot
	ki.Opts.Localversion = "-" + strings.Split(paths.KBootName, ".")[0]
	ki.Opts.Compression = Xz
	ki.Opts.Initramfs = paths.InitramfsBoot
	return ki.BuildKernel()
}
...
func (Kernel) Linuxmfg(ctx context.Context) error {
	...
	ki.Dest = paths.KMfg
	ki.Opts.Localversion = "-" + strings.Split(paths.MfgName, ".")[0]
	ki.Opts.Compression = Xz
	ki.Opts.Initramfs = paths.InitramfsMfg
	return ki.BuildKernel()
}
...
func FindKernel() (*KInfo, error) {
	matches, err := fp.Glob(fp.Join(paths.RepoRoot, "build/linux-*.config"))
	if err != nil {
		return nil, err
	}
	if len(matches) != 1 {
	...
	cfg := fp.Base(matches[0])
	ver := strings.TrimSuffix(cfg, ".config")
	src := fp.Join(paths.WorkDir, ver)
...
func getKSrc(path string) error {
	ver := fp.Base(path)
	fname := ver + ".tar.xz"
	tballPath := fp.Join(paths.WorkDir, fname)
	blobpath := paths.BlobstoreKDir + fname
	...
	//extract
	err = sh.Run("tar", "xJf", tballPath, "-C", paths.WorkDir)
	if err != nil {
