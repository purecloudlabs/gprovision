...
func qemu(ctx context.Context) error {
	...
	if present {
		...
		paths.QemuImg = "qemu-img"
		return nil
	}
	tball := fp.Join(paths.WorkDir, "qemu.txz")
	if _, err := os.Stat(tball); err == nil {
		//tarball exists
		return wrapIf("extracting tarball", sh.Run("tar", "xJf", tball, "-C", paths.QDir))
	}
	//try to get from blobstore
	if err = blobcp(paths.BlobstoreQemu, tball, false); err == nil {
		return wrapIf("extracting tarball", sh.Run("tar", "xJf", tball, "-C", paths.QDir))
	}
...
func blobrm(path string) error {
	if len(paths.BlobstorePrefix) == 0 {
		fmt.Println("skipping blob op, no prefix set")
		return nil
	}
	u, err := url.Parse(paths.BlobstorePrefix)
	if err != nil {
		fmt.Printf("parsing %s: %s\n", paths.BlobstorePrefix, err)
		return err
	}
	switch u.Scheme {
	case "s3":
...
func qemuEnv(kernel string, uefi bool) []string {
	...
	env = append(env, fmt.Sprintf("%s=%s", "QEMU_IMG", paths.QemuImg))
	if uefi {
		env = append(env, fmt.Sprintf("%s=%s", "OVMF_DIR", paths.OvmfDir))
	}
