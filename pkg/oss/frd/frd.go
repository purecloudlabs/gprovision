...
type Frjson struct {
	//Preserve: if true, do not delete this file after use
	Preserve bool
	// IgnoreNetCfg: if true, delete any saved network config rather than using it
	IgnoreNetCfg bool
	// ClearHistory: if true, start fresh file for recovery history
	ClearHistory bool
	// XLog: external logging endpoint
	XLog string
	// BootArgs: additional boot args to pass to kernel (applied during factory
	// restore, takes effect next boot)
	BootArgs string
}
