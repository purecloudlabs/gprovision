...
// A function that returns true if the entry is to be kept. relpath is relative
// to dir, which is likely /sys/class/block.
...
func DFiltOnlyParts(dir, relpath string) bool {
	_, err := os.Stat(fp.Join(dir, relpath, "partition"))
	accept := err == nil
	if Verbose {
		log.Logf("DFiltOnlyParts: path=%s accept=%t", fp.Base(relpath), accept)
	}
	return accept
}

func DFiltOnlyUsbParts(dir, relpath string) bool {
	return DFiltOnlyUsb(dir, relpath) && DFiltOnlyParts(dir, relpath)
}
...
func devices(sysdir string, include DevIncludeFn) (devs []string) {
	...
	for _, entry := range dir {
		...
		if include != nil && !include(sysdir, link) {
			continue
		}
