...
import (
	...
	"gprovision/pkg/log"
	"gprovision/pkg/recovery/archive/meta"
...
func findValidUpd(choices []string) bool {
	for _, upd := range choices {
		...
		if !ok {
			if len(choices) > 1 {
				log.Msgf("history check failed for %s", dtag)
				continue
			} else {
				log.Logf("history check failed for %s, applying anyway", upd)
			}
		}
