...
func (Tests) Unit(ctx context.Context) error {
	mg.CtxDeps(ctx, paths, Bins.Generate, libudev)
...
func (Tests) Integ(ctx context.Context) error {
	...
	env := qemuEnv(KNoInitramfs, false)
...
func lifecycleTest(ctx context.Context, uefi bool) error {
	mg.CtxDeps(ctx, paths, Kernel.Linuxmfg, Kernel.Boot, qemu)

	env := qemuEnv(KMfg, uefi)
	lc_img := fmt.Sprintf("%s=%s", "LC_IMG", FakeUpdate)
...
func integArgs(ctx context.Context, onlyRun string) ([]string, error) {
	pkgs := []string{"gprovision/testing/integ/"}
	prop := "gprovision/proprietary/testing/integ/"
	_, err := os.Stat(fp.Join(RepoRoot, "gopath/src", prop))
	if err == nil {
		pkgs = append(pkgs, prop)
	}
...
func libudevEnv() []string {
	return []string{
		"CGO_ENABLED=1",
		"CGO_CFLAGS=-I" + WorkDir,
		"CGO_LDFLAGS=-L" + WorkDir,
...
func (Tests) Lint(ctx context.Context) error {
	...
	exprList := []string{
		...
		"SA9004",
		"`(wrapBrackets|wrapCorners|unknown|endiannessSwap|testPrintComplexCfg|validateCFEX|expectLogContent|defaultRoute[46]|bindata_.*)` is unused",
	...
	lint.Dir = os.Getenv("GOPATH") + "/src/gprovision"
	err = lint.Run()
	if err != nil {
		fmt.Println("golangci-lint:", err.Error())
